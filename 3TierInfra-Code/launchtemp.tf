data "aws_ami" "example" {
  most_recent = true
  owners      = ["545009838510"] # Replace with the AWS account ID if needed

  filter {
    name   = "name"
    values = ["frontman"] # Use your AMI name pattern
  }
}
...
resource "aws_launch_template" "frontend" {
  ...
  key_name = "Ironman123" #chnage the key 
...
data "aws_ami" "example1" {
  ...
  filter {
    name   = "name"
    values = ["backman"] # Use your AMI name pattern
  }
}
